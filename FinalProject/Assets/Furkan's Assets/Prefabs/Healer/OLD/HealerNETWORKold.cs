...
public class HealerNETWORKold : NetworkBehaviour
{
    ...
    void Attack()
    {
        GameObject localBullet = Instantiate(bullet, bulletSpawnPos.transform.position, bulletSpawnPos.transform.rotation);
        localBullet.GetComponent<Rigidbody>().velocity = bulletSpawnPos.transform.forward * bulletSpeed;
        if (hasAuthority) localBullet.AddComponent<Bullet_Healer>();
    }
    ...
    void Ult()
    {
        GameObject localBullet = Instantiate(UltiBullet,GetMousePosition(),Quaternion.identity);
        //if (hasAuthority) localBullet.AddComponent<Ult_Healer>();
    }
